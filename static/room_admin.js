...
function openRoomModal(mode, editBtn) {
    ...
    form.reset();
    modal.querySelectorAll('.error-text').forEach(el => { el.textContent = ''; });
    modal.querySelectorAll('.alert-danger').forEach(el => { el.style.display = 'none'; });
    ...
    if (mode === 'create') {
        ...
        // 全チェックボックスを未選択に戻す
        modal.querySelectorAll('input[name="facilities"]').forEach(cb => { cb.checked = false; });
        modal.querySelectorAll('input[name="departments"]').forEach(cb => { cb.checked = false; });
        ...
        document.getElementById('id_capacity').value = editBtn.dataset.capacity || '';
        document.getElementById('id_floor').value    = editBtn.dataset.floor    || '';

        // 建物セレクトの復元
        document.getElementById('id_building').value = editBtn.dataset.buildingId || '';

        // 設備チェックボックスの復元
        const facilityIds = editBtn.dataset.facilityIds
            ? editBtn.dataset.facilityIds.split(',').map(s => s.trim()).filter(Boolean)
            : [];
        ...
        const deptIds = editBtn.dataset.departmentIds
            ? editBtn.dataset.departmentIds.split(',').map(s => s.trim()).filter(Boolean)
            : [];
