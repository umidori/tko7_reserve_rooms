...
class TestF13DuplicateCheck(TestCase):
    ...
    def setUp(self):
        ...
        self.other_room = Room.objects.create(name='会議室B', capacity=5, is_active=True)
    ...
    def test_no_overlap_after_existing_succeeds(self):
        ...
        self.assertEqual(
            Reservation.objects.filter(is_cancelled=False).count(), 2
        )
    ...
    def test_cancelled_existing_not_counted_as_overlap(self):
        ...
        self.assertEqual(
            Reservation.objects.filter(is_cancelled=False).count(), 1
        )
